...
**Impl.** `ZSTD_decodeLiteralsBlock`, `ZSTD_decodeSeqHeaders{,_probe}`, `ZSTD_buildFSETable{,_body,_body_default,_body_bmi2}` + `ZSTD_buildSeqTable{,_rle}`, `ZSTD_decompressSequences{,_default,_bmi2,Long*,SplitLitBuffer*}`, `ZSTD_safecopy`, `ZSTD_execSequence{,End,SplitLitBuffer,EndSplitLitBuffer}`, `ZSTD_getOffsetInfo`, `ZSTD_prefetchMatch`, `ZSTD_dictionaryIsActive`, `ZSTD_assertValidSequence`, `ZSTD_decompressBlock_internal` (scalar body path). Prefetch / split-lit-buffer / long-offset variants currently forward to the scalar body where the Rust layout doesn't need separate unsafe hot loops. The seq-exec layer carries an explicit `ext_history: &[u8]` parameter through every wrapper down to `ZSTD_execSequence`, so back-references with `offset > oLitEnd` resolve into the upstream `extDict` virtual layout `[stream_dict ++ historyBuffer ++ dst]`; `ZSTD_decompressBlock_internal` builds the concatenation from `ZSTD_DCtx::{stream_dict, historyBuffer}`.
