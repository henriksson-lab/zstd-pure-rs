...
/// predicate for the same generated specialization set.
#[inline(never)]
fn ZSTD_compressBlock_fast_noDict_generic_mls<const MLS: u32, const USE_CMOV: bool>(
    ...
    'search: loop {
        ...
        let mut hash0 = unsafe {
            let p = src.as_ptr().wrapping_add(ip0);
            match MLS {
                5 => ZSTD_hash5((p as *const u64).read_unaligned().to_le(), hlog, 0),
                6 => ZSTD_hash6((p as *const u64).read_unaligned().to_le(), hlog, 0),
                7 => ZSTD_hash7((p as *const u64).read_unaligned().to_le(), hlog, 0),
                8 => ZSTD_hash8((p as *const u64).read_unaligned().to_le(), hlog, 0),
                _ => ZSTD_hash4((p as *const u32).read_unaligned().to_le(), hlog, 0) as usize,
            }
        };
        let mut hash1 = unsafe {
        ...
        let (match0, mut mLength, offcode) = loop {
            ...
            let match0_pos = matchIdx.wrapping_sub(base_off) as usize;
            ...
            // `ip2 + 8 < iend = src.len()`.
            hash1 = unsafe {
                let p = src.as_ptr().wrapping_add(ip2);
                match MLS {
                    5 => ZSTD_hash5((p as *const u64).read_unaligned().to_le(), hlog, 0),
                    6 => ZSTD_hash6((p as *const u64).read_unaligned().to_le(), hlog, 0),
                    7 => ZSTD_hash7((p as *const u64).read_unaligned().to_le(), hlog, 0),
            ...
            let match0_pos2 = matchIdx.wrapping_sub(base_off) as usize;
            ...
            if match0_found2 {
                ...
                let mut match0 = matchIdx.wrapping_sub(base_off) as usize;
                rep_offset2 = rep_offset1;
                rep_offset1 = (ip0 - match0) as u32;
                let offcode = OFFSET_TO_OFFBASE(rep_offset1);
                let mut mLength = 4;
        ...
        if ip0 <= ilimit {
            ...
            // SAFETY: upstream reaches this block only when `ip0 <= ilimit`;
            // `current0 + 2` is the pre-match `ip0 + 2`, which is still
            // within the same matchable block window.
            let h = unsafe {
                let p = src.as_ptr().wrapping_add(curr_plus2_pos);
                match MLS {
                    5 => ZSTD_hash5((p as *const u64).read_unaligned().to_le(), hlog, 0),
                    6 => ZSTD_hash6((p as *const u64).read_unaligned().to_le(), hlog, 0),
                    7 => ZSTD_hash7((p as *const u64).read_unaligned().to_le(), hlog, 0),
            ...
            // SAFETY: `ip0 <= ilimit = iend - 8` gives `ip0 - 2 + 8 <= iend`;
            // after a stored match, `ip0 >= anchor + 4`, so `ip0 >= 2`.
            let h = unsafe {
                let p = src.as_ptr().wrapping_add(ip0 - 2);
                match MLS {
                    5 => ZSTD_hash5((p as *const u64).read_unaligned().to_le(), hlog, 0),
                    6 => ZSTD_hash6((p as *const u64).read_unaligned().to_le(), hlog, 0),
                    7 => ZSTD_hash7((p as *const u64).read_unaligned().to_le(), hlog, 0),
            ...
            while ip0 <= ilimit && rep_offset2 > 0 {
                ...
                let rval = unsafe {
                    (src.as_ptr()
                        .wrapping_add(ip0.wrapping_sub(rep_offset2 as usize))
                        as *const u32)
                        .read_unaligned()
                };
                if cval != rval {
                ...
                let h = unsafe {
                    ...
                    match MLS {
                        ...
                        8 => ZSTD_hash8((p as *const u64).read_unaligned().to_le(), hlog, 0),
                        _ => {
                            ZSTD_hash4((p as *const u32).read_unaligned().to_le(), hlog, 0) as usize
                        }
                    }
                };
...
pub fn ZSTD_compressBlock_fast_dictMatchState_generic(
    ...
    debug_assert!(prefixStartIndex >= ms.window.base_offset);
    let prefixStart = prefixStartIndex.wrapping_sub(ms.window.base_offset) as usize;
    let endIndex = ms.window.base_offset.wrapping_add(src.len() as u32);
    let dms = ms
        .dictMatchState
        .as_deref()
        .expect("ZSTD_compressBlock_fast_dictMatchState requires dictMatchState");
    ...
    let dictStart = dictStartIndex.wrapping_sub(dictBaseOffset) as usize;
    let dictIndexDelta = prefixStartIndex.wrapping_sub(dictEndIndex);
        ...
        loop {
            ...
            let repMatch = if repInDict {
                repIndex
                    ...
                    .wrapping_sub(dictBaseOffset) as usize
            } else {
            ...
            if dictTagsMatch {
                ...
                let dictMatch = dictMatchIndex.wrapping_sub(dictBaseOffset) as usize;
...
pub fn ZSTD_compressBlock_fast_extDict_generic_with_start(
    ...
    match ms.cParams.minMatch {
        5 => ZSTD_compressBlock_fast_extDict_generic_with_start_mls::<5>(
            ms, seqStore, rep, src, istart,
        ),
        6 => ZSTD_compressBlock_fast_extDict_generic_with_start_mls::<6>(
            ms, seqStore, rep, src, istart,
        ),
        7 => ZSTD_compressBlock_fast_extDict_generic_with_start_mls::<7>(
            ms, seqStore, rep, src, istart,
        ),
        _ => ZSTD_compressBlock_fast_extDict_generic_with_start_mls::<4>(
            ms, seqStore, rep, src, istart,
        ),
    }
}
...
#[inline(never)]
fn ZSTD_compressBlock_fast_extDict_generic_with_start_mls<const MLS: u32>(
    ms: &mut ZSTD_MatchState_t,
    seqStore: &mut SeqStore_t,
    rep: &mut [u32; ZSTD_REP_NUM],
    src: &[u8],
    ...
    'outer: loop {
        ...
        let mut hash0 = ZSTD_fastHash::<MLS>(src, ip0, hlog);
        ...
        'search: loop {
            ...
            let rval = if rep_candidate {
                if repInDict {
                    debug_assert!(repLocal + 4 <= dictEnd);
                    unsafe { (dict.as_ptr().wrapping_add(repLocal) as *const u32).read_unaligned() }
                } else {
                    debug_assert!(repLocal + 4 <= src.len());
                    unsafe { (src.as_ptr().wrapping_add(repLocal) as *const u32).read_unaligned() }
                }
            } else {
                ...
                if ip0 <= ilimit {
                    ...
                    while ip0 <= ilimit {
                            ...
                            }
                        {
                            ...
                            ms.hashTable[ZSTD_fastHash::<MLS>(src, ip0, hlog)] =
                                base_off.wrapping_add(ip0 as u32);
                            ip0 += repLength2;
                            anchor = ip0;
                            continue;
            ...
            hash0 = hash1;
            hash1 = ZSTD_fastHash::<MLS>(src, ip2, hlog);
            ip0 = ip1;
            ip1 = ip2;
            ip2 = ip3;
...
mod tests {
    ...
    fn compress_block_fast_with_history_uses_prior_block_content() {
        ...
        seqStore.reset();
        let last = ZSTD_compressBlock_fast_noDict_generic(
            &mut ms,
            &mut seqStore,
            &mut rep,
            &full,
    ...
    fn dict_and_ext_wrappers_route_through_live_fast_entries() {
        ...
        for (idx, f) in variants.into_iter().enumerate() {
            ...
            if idx == 0 {
                ...
                dms.nextToUpdate = ZSTD_WINDOW_START_INDEX;
    ...
    #[test]
    fn fast_dict_match_state_empty_history_starts_at_second_byte_like_c() {
        let cp = ZSTD_compressionParameters {
            windowLog: 17,
            hashLog: 12,
            minMatch: 4,
        ...
        dms.window.dictLimit = ZSTD_WINDOW_START_INDEX;
        dms.window.lowLimit = ZSTD_WINDOW_START_INDEX;
        dms.window.nextSrc = ZSTD_WINDOW_START_INDEX;

        let mut ms = ZSTD_MatchState_t::new(cp);
        ms.window.base_offset = ZSTD_WINDOW_START_INDEX;
        ms.window.dictLimit = ZSTD_WINDOW_START_INDEX;
        ms.window.lowLimit = ZSTD_WINDOW_START_INDEX;
        ms.window.nextSrc = ZSTD_WINDOW_START_INDEX + 64;
        ms.dictMatchState = Some(Box::new(dms));

        let src = b"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
        let mut seq = SeqStore_t::with_capacity(128, 4096);
        let mut rep = [0u32, 0, 0];

        let last_lits = ZSTD_compressBlock_fast_dictMatchState(&mut ms, &mut seq, &mut rep, src);

        assert!(last_lits <= src.len());
        assert_eq!(rep[0], 0);
        assert_eq!(rep[1], 0);
    }
    ...
    #[test]
    fn fast_ext_dict_ip2_repcode_rejects_prefix_start_boundary() {
        let dict = b"0123456789abcdef".to_vec();
        let src = b"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa".to_vec();
        ...
        let mut seq = SeqStore_t::with_capacity(128, 4096);
        let mut rep = [2u32, 4, 8];
        let _last_lits = ZSTD_compressBlock_fast_extDict(&mut ms, &mut seq, &mut rep, &src);

        assert!(
            !seq.sequences
                .iter()
                .any(|s| s.litLength == 2 && s.offBase == REPCODE_TO_OFFBASE(1)),
            "extDict ip2 repcode accepted repIndex == prefixStartIndex"
        );
    }

    #[test]
    fn fast_ext_dict_uses_window_dict_base_for_index_translation() {
        let mut dict = b"0123456789abcdef01234MATCH-window-dict-base-translation".to_vec();
        dict.extend_from_slice(b"xxxxnot-the-same-trailing-buffer");
        let dict_base = ZSTD_WINDOW_START_INDEX + 1000;
        let dict_limit = dict_base + 56;
        let src = b"MATCH-window-dict-base-translation".to_vec();
        ...
        ms.window.nextSrc = dict_limit + src.len() as u32;
        ms.loadedDictEnd = dict_limit;
        ...
        assert!(
            !seq.sequences.is_empty(),
            "extDict path failed to find the dictionary match"
        );
        assert_eq!(OFFBASE_TO_OFFSET(seq.sequences[0].offBase), 35);
    }
}
